...
$.materialnote.addPlugin({
    ...
    buttons : {
        // "cleaner"  is button's namespace.
        "cleaner" : function(lang, options) {
            // make icon button by template function
            return tmpl.iconButton(options.iconPrefix + 'description', {
    ...
    events : {
        'cleaner':function(context){
            ...
            if(options.cleaner.action=='both'||options.cleaner.action=='button'){
                context.memo('button.cleaner',function(){
                    var button=ui.button({
                        ...
                        click:function(){
                            if($note.materialnote('createRange').toString()){
                                var text=cleanText($note.materialnote('createRange').toString(),options.cleaner.newline);
                                $note.materialnote('pasteHTML',text);
                            }else{
                                var text=cleanText($note.materialnote().text(),options.cleaner.newline);
                                $note.materialnote('code',text);
                            }
                            if(options.cleaner.notTime>0){
                                $editor.find('.note-editing-area').append('<div class="materialnote-cleanerAlert alert alert-success" style="'+options.cleaner.notStyle+'">'+lang.cleaner.not+'</div>');
                                setTimeout(function(){$editor.find('.materialnote-cleanerAlert').remove();},options.cleaner.notTime);
                            }
                        }
                    });
            ...
            this.events={
                'materialnote.paste':function(we,e){
                    if(options.cleaner.action=='both'||options.cleaner.action=='paste'){
                        ...
                        $note.materialnote('pasteHTML',text);
                        if(options.cleaner.notTime>0){
                            $editor.find('.note-resizebar').append('<div class="materialnote-cleanerAlert alert alert-success" style="'+options.cleaner.notStyle+'">'+lang.cleaner.not+'</div>');
                            setTimeout(function(){$editor.find('.materialnote-cleanerAlert').remove();},options.cleaner.notTime);
                        }
                    }
                }
